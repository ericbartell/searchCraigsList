...
def runexit(newestResultId,name):
    if newestResultId != None:
        with open("mostRecentListingID_%s.txt" % name, 'w') as f:
...
def prep():
    ...
    print("imports done")
    SLACK_TOKEN = "None"
    with open("token_%s.txt" % name) as f:
...
def manhattanDist(point1, point2):
    ...
    #0.008: .53 miles: .015 dy/mile 
    #~0, .008: .41 miles: .0195 dx/mile
    return(((dy/.015)**2 + (dx/.0195)**2) **(.5))
...
def do_scrape(args):
    ...
    name=args[9]
    newFileName = "allGoodListingIDsAndNames_withPriceAndDate_%s.txt" % name
    #lastId = args[5]
    #ids = args[6]
    requests = args[7]
    bs = args[8]
    with open("mostRecentListingID_%s.txt"%name,'r') as f:
    ...
    from craigslist import CraigslistHousing
    cl = CraigslistHousing(site='boston', area='gbs', category='aap',
                             filters={'max_price': 1701, 'min_price': 1000, 'max_bedrooms': 1, 'bundle_duplicates':True})
    ...
    #cambridge t stops
    centralsq = (42.365156, -71.103689)
    harvardsq = (42.373490, -71.119041)
    kendalsq = (42.362368, -71.086187)
    #acceptable walk: 3-4 miles
    
    #define lines
    pointPairs = [] #null value, change if we want to define a region
    #pointPairs = [[misHil, fen, 'pos'],[fen, beaconEast, 'pos'], #
    #              [beaconWest, pondSouth, 'neg'],[beaconEast, beaconWest, 'neg'],[pondSouth, misHil, 'pos']]

    circleCenters = [centralsq,harvardsq,kendalsq]
...
##                kitchenFlag = True
            ...
            if result["geotag"] != None:
                a, b = result["geotag"]
                distFromCLS=manhattanDist((a,b),CLS)
                flags += "-%s miles to work-" % (str(distFromCLS)[0:3] if len(str(distFromCLS)) > 3 else str(distFromCLS))
                if distFromCLS > 4.5:
                    bad = True
                else:
                    bad = False
...
##                        if ((a-tcenter[0])**2 + (b-tcenter[1])**2) < 0.0001946025: #12 minute map direct walk
...
##                            #print((a,b))
                ...
                if result["where"] == None:
                    flags += "-no location-"
                    bad = False
                elif ("brookline" in result["where"].lower()# or "brighton" in result["where"].lower()
                      ):
                    flags +="-no geotag-"
                    bad = False
                else:
...
##                            break
                ...
                for item in ['id','name']:
                    if result[item] in ids:
                        ...
                        if result["price"] != ids[result[item]][0]:
                            different = True
                            flags = "-priceChange: $%s-" % (int(ids[result[item]][0].split("$")[1]) - int(result["price"].split("$")[1])) + flags
                ...
                if notRepeat:
                    ...
                    if (splitDate[1] == "aug") or (splitDate[1] == "jul") or (splitDate[1] == "jun") or (splitDate[1] == "may"):
                        goodDates += 1
                        sc.api_call("reactions.add",channel=str(response['channel']),timestamp=str(response['ts']),name="star")
                    #if laundryInUnit:
